...
const createBlocks = () => {
    ...
    for (let i = 0; i < config.blockCount; i++) {
        ...
        const newBlock = Matter.Bodies.fromVertices(x, Math.random() * (window.innerHeight * -0.2) - blockSize * scale, vertices, {
            icon,
...
for (const elem of document.querySelectorAll('.word')) {
    ...
    const newBody = Matter.Bodies.rectangle(bounds.x + bounds.width * 0.5, bounds.y + bounds.height * 0.5 + window.scrollY, bounds.width, bounds.height, {
        elem,
        isStatic: true,
    });
...
syncWords();
const ceiling = Matter.Bodies.rectangle(window.innerWidth * 0.5, window.innerHeight * -1 - thickness * 0.5, window.innerWidth * 10, thickness, {
    isStatic: true,
});
const ground = Matter.Bodies.rectangle(window.innerWidth * 0.5, window.innerHeight + thickness * 0.5, window.innerWidth * 10, thickness, {
    isStatic: true,
});
const wallRight = Matter.Bodies.rectangle(window.innerWidth + thickness * 0.5, 0, thickness, window.innerHeight * 10, {
    isStatic: true,
});
const wallLeft = Matter.Bodies.rectangle(thickness * -0.5, 0, thickness, window.innerHeight * 10, {
    isStatic: true,
});
const syncWalls = () => {
...
const handleDeviceOrientation = ({ beta, gamma }) => {
    const xGravity = gsap.utils.clamp(-config.xLimit, config.xLimit, gsap.utils.mapRange(-90, 90, -config.xLimit, config.xLimit, gamma));
    const yGravity = gsap.utils.clamp(-config.yLimit, config.yLimit, gsap.utils.mapRange(-90, 90, -config.yLimit, config.yLimit, beta));
    config.y = yGravity;
    config.x = xGravity;
    ctrl.refresh();
};
gravity
    ...
    if (config.device) {
        if (DeviceOrientationEvent === null || DeviceOrientationEvent === void 0 ? void 0 : DeviceOrientationEvent.requestPermission) {
            Promise.all([DeviceOrientationEvent.requestPermission()]).then((results) => {
